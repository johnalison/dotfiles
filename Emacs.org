...
[[https://github.com/seagle0128/grip-mode][grip-mode]] renders Markdown using GitHub's API, giving a pixel-perfect GitHub preview in the browser.

#+begin_src emacs-lisp

  (use-package grip-mode
    :straight t
    :init (add-to-list 'exec-path (expand-file-name "~/.local/bin")))
...
Generate git commit messages via gptel instead of copilot.

#+begin_src emacs-lisp

  (defvar my/commit-message-buffer nil
    "Buffer into which the next gptel commit message response will be inserted.")

  (defvar my/commit-message-placeholder nil
    "Placeholder string inserted while waiting for the gptel commit message response.")

  (defvar my/commit-message-marker nil
    "Marker tracking the streaming insertion point in the commit buffer.")

  (defconst my/commit-diff-max-chars 8000
    "Maximum characters of diff to send to the LLM.")

  (defun my/insert-commit-message ()
    "Generate a git commit message via gptel and insert it into the commit buffer."
    (let ((diff (shell-command-to-string "git diff --cached")))
      (when (string-empty-p diff)
        (setq diff (shell-command-to-string "git diff HEAD")))
      (if (string-empty-p diff)
          (message "No staged changes found for commit message generation.")
        (when (> (length diff) my/commit-diff-max-chars)
          (setq diff (concat (substring diff 0 my/commit-diff-max-chars)
                             "\n... [diff truncated]")))
        (setq my/commit-message-buffer (current-buffer))
        (setq my/commit-message-placeholder
              (format "# Generating commit message from %s...\n" gptel-model))
        (setq my/commit-message-marker (make-marker))
        (goto-char (point-min))
        (insert my/commit-message-placeholder)
        (set-marker my/commit-message-marker (point-min))
        (gptel-request
         (concat
          ...
          "and add a blank line followed by bullet-point details if the change warrants it.\n"
          "Output only the commit message text, no commentary.\n\n"
          diff)
         :stream t
         :callback (lambda (response info)
                     (with-current-buffer my/commit-message-buffer
                       (goto-char (point-min))
                       (when (looking-at (regexp-quote my/commit-message-placeholder))
                         (delete-region (point) (match-end 0))
                         (set-marker my/commit-message-marker (point-min)))
                       (cond
                         ((stringp response)
                          (goto-char my/commit-message-marker)
                          (insert response)
                          (set-marker my/commit-message-marker (point)))
                         ((not response)
                          (message "gptel commit message error: %s" (plist-get info :status))))))))))

  (add-hook 'git-commit-setup-hook 'my/insert-commit-message)
...
#+begin_src emacs-lisp
    ...
    (use-package gptel
      ...
      (gptel-make-preset 'min-rewrite
        :description "minimal rewrite"
        :system "Can you rewrite for spelling and missed words. Don't change structure or phrasing. Please keep formatting roughly the same."
        :use-context 'system)

      (gptel-make-preset 'email
        :description "Draft emails in John's voice"
        :system "Draft or rewrite the following as an email in my voice. Rules:
  - Greetings: 'Hi [First Name],' for known contacts; 'Dear Colleagues,' for groups; no greeting for terse thread replies
  - Closings: 'john' (lowercase) for substantive emails; 'Thanks' for short replies; nothing for very short ones. Never: 'Best regards', 'Sincerely', 'Best', 'Looking forward to hearing from you'
  - Length: 1-5 sentences for most replies; 2-3 short paragraphs max for multi-part questions
  - No preamble, no pleasantries, jump straight to content
  - Answer multi-part questions point-by-point inline, no bullet points
  - Contractions fine; minimal exclamation marks
  - Never write: 'I hope this email finds you well', 'Thank you for reaching out', 'Looking forward to hearing from you', or any formal closing")
...
(use-package monet
  :straight (:type git :host github :repo "stevemolitor/monet" :depth 1)
  :demand t
  :custom
  (monet-prefix-key "C-c o"))
...
#+begin_src emacs-lisp
    ...
    (use-package dired
      ...
      :custom ((insert-directory-program (if (eq system-type 'darwin) "gls" "ls"))
...
** Send region to wiki inbox

Appends the selected region as an unprocessed idea to =~/ClaudeBrain/wiki-inbox.md=,
under a =## YYYY-MM-DD= date section (creating it if needed). Bound to =C-c w i=.

#+begin_src emacs-lisp :tangle ~/dotfiles/init-org.el

  (defun my/send-region-to-wiki-inbox (beg end)
    "Append the selected region to ~/ClaudeBrain/wiki-inbox.md as a - [ ] item.
  Inserts a date heading (## YYYY-MM-DD) if today's section doesn't exist yet."
    (interactive "r")
    (let* ((text (string-trim (buffer-substring-no-properties beg end)))
           (date (format-time-string "%Y-%m-%d"))
           (heading (concat "## " date))
           (inbox-path (expand-file-name "~/ClaudeBrain/wiki-inbox.md"))
           (item (concat "- [ ] " text)))
      (with-current-buffer (find-file-noselect inbox-path)
        (goto-char (point-max))
        ;; Check if today's heading exists anywhere in the file
        (if (save-excursion (goto-char (point-min))
                            (search-forward heading nil t))
            ;; Heading exists — move to end of that section
            (progn
              (goto-char (point-min))
              (search-forward heading)
              (end-of-line)
              ;; Skip forward past existing items in this section
              (while (and (not (eobp))
                          (progn (forward-line 1)
                                 (or (looking-at "^- ")
                                     (looking-at "^$"))))
                nil)
              ;; Back up to insert after the last item/blank line
              (forward-line -1)
              (end-of-line)
              (insert "\n" item))
          ;; No heading for today — append at end of file
          (goto-char (point-max))
          (unless (bolp) (insert "\n"))
          (insert "\n" heading "\n" item "\n"))
        (save-buffer))
      (message "Sent to wiki inbox: %s" text)))

  (global-set-key (kbd "C-c w i") #'my/send-region-to-wiki-inbox)
...
* Agenda Store ID Link

From the agenda, store a permanent =id:= link to the task headline.
Bound to =C-c s= in agenda. Link is =[[id:UUID][Heading Text]]= format —
safe across file moves (Tasks.org → Archive.org) and carries a readable
description. Links persist in the list after insertion.

#+begin_src emacs-lisp :tangle ~/dotfiles/init-org.el

  ;; Keep stored links available after insertion so the same link
  ;; can be reused across multiple notes.
  (setq org-keep-stored-link-after-insertion t)

  (defun my/org-agenda-store-id-link ()
    "From org-agenda, jump to the task headline, store a permanent id: link,
  then return to the agenda.  Use C-c C-l to insert the link in any org buffer.
  The link description is the heading text (tags, TODO, priority stripped)."
    (interactive)
    (unless (derived-mode-p 'org-agenda-mode)
      (user-error "This must be called from an org-agenda buffer"))
    (save-window-excursion
      (org-agenda-switch-to)
      (let* ((id   (org-id-get-create))
             (desc (or (org-get-heading t t t t) id)))
        (push (list (concat "id:" id) desc) org-stored-links)))
    (message "ID link stored — use C-c C-l to insert it."))

  (define-key org-agenda-mode-map (kbd "C-c s") #'my/org-agenda-store-id-link)
...
#+begin_src emacs-lisp :tangle ~/dotfiles/init-org.el
  ...
  (run-at-time t (* 8 3600) (lambda () (let ((inhibit-message t)) (org-gcal-sync))))
...
#+begin_src emacs-lisp :tangle ~/dotfiles/init-org.el
  ...
  (setq org-export-with-title t
        ...
        org-html-validation-link nil     ;; remove the W3C validation link everywhere
        org-html-postamble nil   ;; remove the entire footer (postamble), which is where “Created:” lives
        )
...
#+begin_src emacs-lisp :tangle ~/dotfiles/init-org.el
  ...
  ;; Make ~ red text.
  ;;
  (add-to-list 'org-emphasis-alist '("~" (:foreground "red") nil))
...
#+begin_src emacs-lisp
  ...
  (use-package telega
    ...
    :config
    (define-key telega-chat-mode-map (kbd "<backtab>") 'dabbrev-expand))
